...
export class Router {
    ...
    async activateRoute(e, oldRoute = null) {
        if (oldRoute) {
            //удаление файлов стилей от старого роута
            ...
            //удаление скриптов от старого роута
        ...
        if (newRoute) {
            //подключение файлов стилей для нового роута
            if (newRoute.styles && newRoute.styles.length > 0) {
                newRoute.styles.forEach(style => {
                    FileUtils.loadPageStyle('/css/' + style, this.adminLteStyleElement)
                })
            }
            //подключение скриптов нового роута (promise)
